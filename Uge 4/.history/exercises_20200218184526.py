...
def getSumPerSpecificHoods():
    ...
    west = {}
    east = {}
    for yearkey in years.items():
        for hoodkey in specificHoods.items():
            if(hoodkey == 2):
                east[yearkey] = getPopPerSpecificHood(yearkey, hoodkey)
            else:
                west[yearkey] = getPopPerSpecificHood(yearkey, hoodkey)
            # print(value, getPopPerSpecificHood(key))
            # lst.append({value: getPopPerSpecificHood(key)})
    lst.append(west,east)
    return lst
