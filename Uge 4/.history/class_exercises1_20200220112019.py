...
def sumPop(aar=None, bydel=None, statkode=None, alder=None,):
    ...
    if(aar is None):
        mask = (dd[:,1] == bydel) & (dd[:,2]== alder) & (dd[:,3] == statkode)
        print("Pop data without year parameter:\n",dd[mask],"\nPop count:\n", np.mean(dd[mask][:,4]) )
    if(statkode is None and bydel is None):
        mask = (dd[:,0] == aar) & (dd[:,2] == alder)
        print("Pop data without citizenship or area parameter:\n",dd[mask],"\nPop count:\n", np.sum(dd[mask][:,4]) )
...
# sumPop(aar = 2015, alder = 99)
sumPop(aar=2015,bydel=1, alder=15, statkode=5181)
# sumPop(aar=2015,bydel=1, statkode=5180)
# sumPop(2015,1,5180)

# sumPop(bydel=1, alder=15, statkode=5180)
